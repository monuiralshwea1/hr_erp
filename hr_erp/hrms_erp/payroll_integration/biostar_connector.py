...
"""
موصل BioStar — biostar_connector.py
=====================================

منقول من تطبيق navari-frappehr-biostar (نسخة version-15) مع تعديل:
  - base_url يُمرَّر كمعامل بدل قراءته من DocType "Biostar Settings"
  - تحديث last_server_status يُخزَّن في "HR ERP Payroll Settings"

المرجع: https://github.com/navariltd/navari-frappehr-biostar
...
class BiostarConnector:
	...
	def login(self):
		if not self.check_server_status():
			frappe.db.set_value(SETTINGS_DOCTYPE, SETTINGS_DOCTYPE, "last_server_status", "Offline") if frappe.get_meta(SETTINGS_DOCTYPE).has_field("last_server_status") else None
			return
	...
	def get_attendance_ids(self, employees=None):
		...
		attendance_ids = [emp.attendance_device_id for emp in employee_data if emp.attendance_device_id]
	...
	def get_attendance_report(self, attendance_ids, start_date, end_date):
		...
		try:
			response = requests.post(
				attendance_url, data=json.dumps(request_body), headers=headers, verify=False
			)
			response.raise_for_status()
			if response.status_code == 200:
				...
				self.attendance_logs = [
					log for log in records if log.get("inTime") != "-" or log.get("outTime") != "-"
				]
		except Exception as e:
		...
		if self.attendance_logs:
			...
			for log in self.attendance_logs:
				if log["in_time"] != "-":
					...
					log["datetime_in"] = datetime.strptime(datetime_str, datetime_format)
					self.punch_logs.append({
				...
				if log["out_time"] != "-":
					...
					log["datetime_out"] = datetime.strptime(datetime_str, datetime_format)
					self.punch_logs.append({
						"employee_field_value": log["employee_field_value"],
	...
	def update_last_sync_employee_date(self):
		...
		for emp in employees:
			updates[emp.name] = {"custom_last_attendance_sync_date": end_date}
	...
	def set_last_sync_of_checkin_as_now(self):
		...
		for shift_type in shift_types:
			frappe.db.set_value("Shift Type", shift_type, "last_sync_of_checkin", datetime.now(), update_modified=False)
		frappe.db.commit()


def add_checkin_data(punch_logs, end_date):
	frappe.enqueue(
		"hr_erp.hrms_erp.payroll_integration.biostar_connector.create_employee_checkins",
		queue="long",
...
def create_employee_checkins(punch_logs, end_date=None):
	...
	for punch_log in punch_logs:
		...
		try:
			...
			frappe.db.rollback()
			frappe.log_error(
				f"Failed checkin for {punch_log.get('employee_field_value')} at {punch_log.get('timestamp')}. Error: {str(e)}"
			)
